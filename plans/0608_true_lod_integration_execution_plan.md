...
- `0608_1448` Plan B B1/B2/B3 implementation completed: added `finetune_stf/dataset/lod_aug.py`; wired train-only LOD augmentation into `finetune_stf/dataset/lod_true.py`; added explicit `--aug-*`, `--aug-preset`, `--eval-lod-train-proxy`, `--lod-train-proxy-count`, `--lr-schedule`, and `--warmup-steps` handling in `finetune_stf/train.py`; removed hidden train-loop runtime hflip for LOD only while preserving ROD/STF behavior.
- `0608_1448` Plan B implementation details: `aug-preset=off` is true no-aug; `baseline_e10` is explicit hflip0.5; `geom` uses hflip0.5 + scale 0.7,1.4; `medium` adds domain-specific photometric. LOD geometric RNG is derived from `(seed, epoch, sample_id, stream)` so RGB/RAW same sample gets paired hflip/scale/crop. `resolved_config.json` now records expanded augmentation and LR schedule payloads, and logs print `[AUG] ...` plus `[LR] schedule=... warmup_steps=...`.
- `0608_1448` Plan B smoke passed: `python -m py_compile` passed for `train.py`, `lod_aug.py`, `lod_true.py`, and `eval_lod_train_val_gap.py`; dataset smoke passed for off passthrough, forced hflip, paired RGB/RAW geometry, and RAW medium finite `[0,1]`; train smokes passed for RGB `geom`, RGB `medium`, RAW `geom`, and RAW `medium` with `cosine/warmup_steps=1`. All successful `/tmp/codex_smoke_lod_aug*` and `/tmp/codex_smoke_lod_gap*` outputs were deleted.
