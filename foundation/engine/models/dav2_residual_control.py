...
def _feature_channels(feature_source: str) -> int:
    if feature_source == "rgb":
        return 4
    if feature_source == "d0":
        return 1
    raise ValueError(f"Unsupported residual_feature_source={feature_source!r}; expected {CONTROL_FEATURE_SOURCES}")


class DAV2ResidualControl(nn.Module):
    """
    Frozen RGB DAv2 baseline refined by a trainable control residual head.
    ...
    def __init__(
        ...
        *,
        residual_feature_source: str = "rgb",
        ...
        if residual_feature_source not in CONTROL_FEATURE_SOURCES:
            raise ValueError(
                f"Unsupported residual_feature_source={residual_feature_source!r}; "
                f"expected one of {CONTROL_FEATURE_SOURCES}"
            )
    ...
    def _head_input(self, *, d0_norm: torch.Tensor, image_rgb_norm: torch.Tensor) -> torch.Tensor:
        d0_norm_ch = d0_norm.unsqueeze(1)
        if self.residual_feature_source == "rgb":
            return torch.cat([d0_norm_ch, image_rgb_norm], dim=1)
        if self.residual_feature_source == "d0":
            return d0_norm_ch
    ...
    def forward(self, batch: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        head_input = self._head_input(d0_norm=d0_norm, image_rgb_norm=image)
    ...
    def load_base_dav2_state_dict(self, state_dict: dict[str, torch.Tensor]):
        ...
        if missing or status.unexpected_keys:
            raise RuntimeError(
                "Failed to load compatible DAv2 base weights for DAV2ResidualControl: "
...
def build_dav2_residual_control_model(
    dav2_model: nn.Module,
    *,
    residual_feature_source: str = "rgb",
    residual_alpha: float = 0.5,
