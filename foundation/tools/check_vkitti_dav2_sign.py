...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check global sign between frozen DAv2 output and VKITTI inverse depth.")
...
def load_fullres_original(
    *,
    ...
    if image is None:
        raise ValueError(f"Failed to read fullres VKITTI image: {image_path}")
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB).astype(np.float32) / 255.0
    image_norm = (np.clip(image, 0.0, 1.0).astype(np.float32, copy=False) - IMAGENET_MEAN) / IMAGENET_STD
    image_tensor = torch.from_numpy(np.transpose(np.ascontiguousarray(image_norm), (2, 0, 1))).unsqueeze(0)

    depth = cv2.imread(str(depth_path), cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
    if depth is None:
        raise ValueError(f"Failed to read fullres VKITTI depth: {depth_path}")
    ...
    return image_tensor, depth, valid
...
def main() -> None:
    ...
    fullres_quality: list[dict[str, float]] = []
    global_x: list[np.ndarray] = []
    global_y: list[np.ndarray] = []
    fullres_original_hw: list[int] | None = None
    processed = 0
    with torch.no_grad():
        for batch in loader:
            ...
            if math.isfinite(corr):
                ...
                fullres_image, fullres_depth, fullres_valid = load_fullres_original(
                    image_path=batch["image_path"][0],
                    depth_path=batch["depth_path"][0],
                    min_depth=args.min_depth,
                    max_depth=args.max_depth,
                )
                fullres_original_hw = [int(fullres_depth.shape[0]), int(fullres_depth.shape[1])]
                fullres_image = fullres_image.to(device, non_blocking=True).float()
                fullres_d0 = model(adapter.pad_rgb(fullres_image))
                fullres_d0 = adapter.crop_depth(fullres_d0)[0].detach().cpu().numpy().astype(np.float32)
                full_metrics = compute_d0_quality(fullres_depth, fullres_valid, fullres_d0, args)
                if full_metrics is not None:
                    fullres_quality.append(full_metrics)
    ...
    fullres_quality_mean = {key: mean_quality(fullres_quality, key) for key in QUALITY_KEYS}
    halfres_tag = f"halfres_{int(args.input_height)}x{int(args.input_width)}"
    fullres_hw_tag = (
        "fullres_original_unknown"
        if fullres_original_hw is None
        else f"fullres_original_{fullres_original_hw[0]}x{fullres_original_hw[1]}"
    )
    payload = {
        ...
        "fullres_original_hw": fullres_original_hw,
        "quality": {
            halfres_tag: halfres_quality_mean,
            fullres_hw_tag: fullres_quality_mean,
        },
    }
    for key in QUALITY_KEYS:
        payload[f"{halfres_tag}_D0_{key}_mean_over_{int(args.max_samples)}"] = halfres_quality_mean[key]
        payload[f"{fullres_hw_tag}_D0_{key}_mean_over_{int(args.max_samples)}"] = fullres_quality_mean[key]
