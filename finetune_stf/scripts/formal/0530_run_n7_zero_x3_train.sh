...
RUN_NAME="${RUN_NAME:-${timestamp}_vkitti_n7_zero_x3_train_lp0p5_q0p3_lfl0p0_vits_half187x621_sceneholdout_Scene20_n1000_seed42_bs${BS}_e${EPOCHS}}"
...
if str(cfg.get("method_id", "")).upper() != "N7":
    raise SystemExit(f"Base config must be N7, got {cfg.get('method_id')!r}")
arg_keys = [
...
cmd.extend([
    "--train-feature-ablation-mode", "zero",
    "--eval-feature-ablation-mode", "zero",
    "--feature-ablation-scope", "both",
    "--feature-ablation-key", "x3",
    "--feature-ablation-seed", str(seed),
    "--experiment-label", "N7-zero-x3-train",
    "--save-path", save_path,
...
mkdir -p "${SAVE_PATH}"
LOG_PATH="${SAVE_PATH}/n7_zero_x3_train.log"
{
...
echo "[DONE] N7 zero x3 train outputs: ${SAVE_PATH}" | tee -a "${LOG_PATH}"
