...
SESSION_PREFIX="${SESSION_PREFIX:-rod_night_teacherrgb_lora_tap_e10}"
...
usage() {
  cat <<'EOF'
Usage:
  bash finetune_stf/scripts/formal/0610_run_rod_night_teacher_rgb_lora_tap_e10_queue.sh
  bash finetune_stf/scripts/formal/0610_run_rod_night_teacher_rgb_lora_tap_e10_queue.sh --audit

Starts one tmux session for:
  ROD teacher RGB LoRA tap r8/a16 + decoder, matching 0605_0139 except rendered RGB input.
...
if [[ "${AUDIT_ONLY}" != "1" && "${1:-}" != "--run-internal" ]]; then
  ...
  tmux new-session -d -s "${session}" \
    "cd '${ROOT}' && ROOT='${ROOT}' EXP_ROOT='${EXP_ROOT}' LOG_ROOT='${LOG_ROOT}' HEAVY_ROOT='${HEAVY_ROOT}' CONDA_BIN='${CONDA_BIN}' CONDA_ENV='${CONDA_ENV}' GPU='${GPU}' PRETRAINED='${PRETRAINED}' ROD_ROOT='${ROD_ROOT}' ROD_MANIFEST='${ROD_MANIFEST}' MASTER_PORT='${MASTER_PORT}' QUEUE_SESSION='${session}' CUDA_VISIBLE_DEVICES='${GPU}' bash '$0' --run-internal 2>&1 | tee -a '${queue_log}'"
  cat <<EOF
...
log_header() {
  ...
  local log="$4"
  {
    ...
    echo "[SEMANTICS] input=teacher_bright_degreen_v1 label_space=inverse_relative recipe_ref=0605_0139_lora_tap_r8a16_decoder_e10"
  } 2>&1 | tee -a "${log}"
}
...
payload = {
    ...
    "rod_teacher_white_percentile": args.rod_teacher_white_percentile,
    "rod_teacher_gamma": args.rod_teacher_gamma,
    "rod_teacher_channel_gains": args.rod_teacher_channel_gains,
...
common_args=(
  ...
  --dataset-family rod_raw_teacher_rgb
  --dataset-input-mode raw24_teacher_rgb
  ...
  --rod-rgb-pipeline teacher_bright_degreen_v1
  --rod-teacher-white-percentile 99.5
  --rod-teacher-gamma 0.454545
  --rod-teacher-channel-gains 1.08 0.95 1.10
...
echo "[SCOPE] ROD night teacher RGB LoRA tap r8/a16 + decoder, epochs=10"
echo "[PRETRAINED] ${PRETRAINED}"
echo "[ROD_ROOT] ${ROD_ROOT}"
echo "[ROD_MANIFEST] ${ROD_MANIFEST}"
echo "[PORT] ${MASTER_PORT}"

run_formal_exp "teacher RGB LoRA tap r8/a16 + decoder" \
  "rod_night_teacherrgb_dav2s_lora_tap_r8a16_decoder_e10" "${MASTER_PORT}" \
