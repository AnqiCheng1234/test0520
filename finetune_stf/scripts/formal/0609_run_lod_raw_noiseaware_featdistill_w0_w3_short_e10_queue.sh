...
C_NORMAL_W0="${C_NORMAL_W0:-/mnt/drive/3333_raw/0000_exp_ckpt/0609_0010_lod_true_raw_normal_rgb16_block8excl10_ram3_dav2s_decoder_e40_W0_aug-baseline_e10_poly/best_model.pth}"
SESSION_PREFIX="${SESSION_PREFIX:-lod_raw_noiseaware_featdistill_w0_w3_short_e10_queue}"
...
usage() {
  cat <<'EOF'
Usage:
  bash finetune_stf/scripts/formal/0609_run_lod_raw_noiseaware_featdistill_w0_w3_short_e10_queue.sh
  bash finetune_stf/scripts/formal/0609_run_lod_raw_noiseaware_featdistill_w0_w3_short_e10_queue.sh --audit

Runs no-LoRA short diagnostic:
  W0, W3, W0_seed123

Overrides:
  GPU=0 MASTER_PORT=29651 EPOCHS=10 BS=8 LAMBDA_FEAT=0.05 bash ...
EOF
}
...
if [[ "${AUDIT_ONLY}" != "1" && "${1:-}" != "--run-internal" ]]; then
  ...
  tmux new-session -d -s "${session}" \
    "cd '${ROOT}' && ROOT='${ROOT}' EXP_ROOT='${EXP_ROOT}' LOG_ROOT='${LOG_ROOT}' HEAVY_ROOT='${HEAVY_ROOT}' CONDA_BIN='${CONDA_BIN}' CONDA_ENV='${CONDA_ENV}' GPU='${GPU}' PRETRAINED='${PRETRAINED}' LOD_ROOT='${LOD_ROOT}' LOD_MANIFEST='${LOD_MANIFEST}' C_DARK_W0='${C_DARK_W0}' C_NORMAL_W0='${C_NORMAL_W0}' MASTER_PORT='${MASTER_PORT}' EPOCHS='${EPOCHS}' BS='${BS}' LAMBDA_FEAT='${LAMBDA_FEAT}' RUN_PREFIX='${queue_timestamp}' QUEUE_SESSION='${session}' CUDA_VISIBLE_DEVICES='${GPU}' bash '$0' --run-internal 2>&1 | tee -a '${queue_log}'"
  cat <<EOF
...
log_header() {
  ...
  local log="$4"
  {
    ...
    echo "[STUDENT_INIT_W0] ${C_DARK_W0}"
    echo "[TEACHER_W0] ${C_NORMAL_W0}"
    echo "[SEMANTICS] matrix=${label} epochs=${EPOCHS} bs=${BS} lambda_feat=${LAMBDA_FEAT} pair_input=RAW_Dark+RAW_normal aug=hflip0.5_only lora=none teacher_recipe=decoder_w0"
  } 2>&1 | tee -a "${log}"
}
...
audit_train_args() {
  local run="$1"
  shift
  local audit_save="/tmp/codex_smoke_lod_noiseaware_w0_audit_${run}"
...
run_train "W0" "${RUN_PREFIX}_W0_lod_raw_pair_decoder_e${EPOCHS}_featnone_localnone" "$((MASTER_PORT + 0))" \
  "${common_args[@]}" --seed 42 "${feat_none_args[@]}" "${local_none_args[@]}"

run_train "W3" "${RUN_PREFIX}_W3_lod_raw_pair_decoder_e${EPOCHS}_featmiddeep_lam${LAMBDA_FEAT/./}_noiseawarev1_gate003_s01" "$((MASTER_PORT + 1))" \
  "${common_args[@]}" --seed 42 "${feat_on_args[@]}" "${local_on_args[@]}"

run_train "W0_seed123" "${RUN_PREFIX}_W0_seed123_lod_raw_pair_decoder_e${EPOCHS}_featnone_localnone" "$((MASTER_PORT + 2))" \
