...
SESSION_PREFIX="${SESSION_PREFIX:-lod_true_raw_rgb16_aug_e40_block8excl10_queue}"
...
run_one() {
  ...
  local run_name="${run_timestamp}_lod_true_raw_rgb16_block8excl10_ram3_dav2s_decoder_e40_${run_id}_aug-${aug_preset}_poly"
  ...
  echo "[FORMAL] log=${log}"
  echo "[FORMAL] matrix=${run_id} link=RAW_RGB16 split=block8_val112_excl10_uniform train=1958 val=112 buffer=160 nearest_val_train_pair_distance_min=11 epochs=40 aug_preset=${aug_preset} lr_schedule=poly warmup_steps=0 train_proxy=112 lod_manifest=${LOD_MANIFEST}"
