...
HELP_LOG="${HELP_LOG:-/tmp/dav2_raw_0603_train_help_codex_smoke.txt}"
INPUT_LOG="${INPUT_LOG:-/tmp/dav2_raw_0603_model_input_codex_smoke.txt}"
LIVE_RAW_LOG="${LIVE_RAW_LOG:-/tmp/dav2_raw_0603_live_raw_model_codex_smoke.txt}"
...
"${CONDA_BIN}" run -n "${CONDA_ENV}" python finetune_stf/train.py \
  --pretrained-from /tmp/dav2_raw_0603_missing_pretrained_codex_smoke.pth \
  --save-path /tmp/dav2_raw_0603_live_raw_model_codex_smoke \
  --eval-kitti \
