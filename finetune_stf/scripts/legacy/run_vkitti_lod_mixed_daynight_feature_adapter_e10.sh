...
FEATURE_ADAPTER_KEYS=(${FEATURE_ADAPTER_KEYS:-x_cat ffm_mid x4})
TS="${TS:-$(date +%m%d_%H%M)}"
RUN_NAME="${RUN_NAME:-${TS}_vkitti_lod_dn_feature_adapter_decoder_644x1008_bs4acc4_ssi_lod3_eth3d_best_e10}"
...
conda run -n "${CONDA_ENV}" torchrun --nproc_per_node="${GPUS}" --master_port "${MASTER_PORT}" finetune_stf/train.py \
  ...
  --lr "${LR}" \
  --bridge-lr "${BRIDGE_LR}" \
  --bridge-feature-keys "${FEATURE_ADAPTER_KEYS[@]}" \
  --no-eval-stf \
  --eval-eth3d \
