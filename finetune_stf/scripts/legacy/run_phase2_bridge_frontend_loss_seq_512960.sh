...
RUN1_NAME="${TS}_phase2_vkitti_only_raw_ram_bridge_frontend_only_512960_bs4acc4_dual_eval_v2calib_loss_ssi_e10"
RUN2_NAME="${TS}_phase2_vkitti_only_raw_ram_bridge_frontend_only_512960_bs4acc4_dual_eval_v2calib_loss_ssi_grad_e10"
...
conda run -n dav3 torchrun --nproc_per_node=1 --master_port 29651 finetune_stf/train.py \
  --encoder vitl \
  --stage vkitti_only \
...
conda run -n dav3 torchrun --nproc_per_node=1 --master_port 29652 finetune_stf/train.py \
  --encoder vitl \
  --stage vkitti_only \
  --input-type raw_ram_bridge \
  --dav2-train-mode none \
