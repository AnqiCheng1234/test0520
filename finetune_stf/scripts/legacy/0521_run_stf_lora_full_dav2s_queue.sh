...
SESSION_PREFIX="${SESSION_PREFIX:-stf_0521_dav2s_pseudo_seq}"
RUN_SMOKES="${RUN_SMOKES:-1}"
START_FORMAL_EXP="${START_FORMAL_EXP:-1}"
END_FORMAL_EXP="${END_FORMAL_EXP:-6}"
...
DAV2S_MANIFEST="${DAV2S_MANIFEST:-/mnt/drive/3333_raw/seeing_through_fog/pseudo_depth_dav2_official_vits_rgb_lut_6216/stf_rgb_lut_manifest_6216.csv}"

usage() {
  cat <<'EOF'
Usage:
  DAV2S_MANIFEST=/path/to/stf_rgb_lut_manifest_6216.csv \
...
Starts one tmux session that:
  ...
  2. runs codex_smoke experiments for DAV2-S exp1 and exp3,
  3. deletes successful codex_smoke artifacts only,
  4. runs formal DAV2-S experiments 1..6 sequentially.

Overrides:
  GPU=0 WAIT_FOR_GPU_IDLE=0 CONDA_BIN=/path/to/conda bash ...
  RUN_SMOKES=0 START_FORMAL_EXP=2 END_FORMAL_EXP=6 bash ...

Notes:
  PRETRAINED initializes the vits student backbone. DAV2S_MANIFEST selects the
  DAV2-S pseudo-label teacher outputs used as the training target.
EOF
}
...
echo "[PRETRAINED_STUDENT] ${PRETRAINED}"
echo "[DAV2S_MANIFEST] ${DAV2S_MANIFEST}"

if [[ "${RUN_SMOKES}" == "1" ]]; then
  smoke_ts="$(date +%m%d_%H%M)"
  run_smoke "smoke exp1 dav2s rgb_lora decoder" "codex_smoke_${smoke_ts}_exp1_dav2s_rgb_lora_decoder" 29681 \
    "${common_args[@]}" "${smoke_args[@]}" "${lora_args[@]}" \
    --input-type rgb_lora \
    --stf-train-target-mode dav2_pseudo \
    --stf-pseudo-manifest "${DAV2S_MANIFEST}" \
    --dav2-train-mode decoder

  smoke_ts="$(date +%m%d_%H%M)"
  run_smoke "smoke exp3 dav2s raw_ram_rgb_lora decoder" "codex_smoke_${smoke_ts}_exp3_dav2s_raw_ram_rgb_lora_decoder" 29683 \
...
run_formal_exp 1 "exp1 dav2s rgb_lora decoder" "stf_train_test_pseudovits_rgb_lora_decoder_e5" 29691 \
...
run_formal_exp 2 "exp2 dav2s rgb full" "stf_train_test_pseudovits_rgb_full_lrd09_e5" 29692 \
  "${common_args[@]}" \
  --input-type rgb \
  --stf-train-target-mode dav2_pseudo \
  --stf-pseudo-manifest "${DAV2S_MANIFEST}" \
...
run_formal_exp 3 "exp3 dav2s raw_ram_rgb_lora decoder" "stf_train_test_pseudovits_raw_ram_rgb_bnclean_identity_lora_decoder_e10" 29693 \
...
run_formal_exp 4 "exp4 dav2s raw_ram_rgb full" "stf_train_test_pseudovits_raw_ram_rgb_bnclean_identity_full_lrd09_e10" 29694 \
...
run_formal_exp 5 "exp5 dav2s raw_ram_rgb_bridge_lora decoder" "stf_train_test_pseudovits_raw_ram_rgb_bnclean_bridge_lora_decoder_e10" 29695 \
...
run_formal_exp 6 "exp6 dav2s raw_ram_rgb_bridge full" "stf_train_test_pseudovits_raw_ram_rgb_bnclean_bridge_full_lrd09_e10" 29696 \
