...
RUN1_NAME="${TS}_phase2_vkitti_only_raw_ram_bridge_frontend_only_512960_bs4acc4_dual_eval"
RUN2_NAME="${TS}_phase2_vkitti_only_raw_ram_bridge_frontend_decoder_512960_bs4acc4_dual_eval"
...
conda run -n dav3 torchrun --nproc_per_node=1 --master_port 29621 finetune_stf/train.py \
  --encoder vitl \
  --stage vkitti_only \
...
conda run -n dav3 torchrun --nproc_per_node=1 --master_port 29622 finetune_stf/train.py \
  --encoder vitl \
  --stage vkitti_only \
  --input-type raw_ram_bridge \
