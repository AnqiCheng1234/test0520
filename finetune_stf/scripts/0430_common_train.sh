...
EXP_ROOT="${EXP_ROOT:-${REPO_ROOT}/finetune_stf/exp_186}"
CONDA_ENV="${CONDA_ENV:-dav3}"
CONDA_EXE="${CONDA_EXE:-$(command -v conda || true)}"
if [[ -z "${CONDA_EXE}" && -x "/home/a5000/anaconda3/bin/conda" ]]; then
  CONDA_EXE="/home/a5000/anaconda3/bin/conda"
fi
if [[ -z "${CONDA_EXE}" && -x "/home/caq/anaconda3/bin/conda" ]]; then
  CONDA_EXE="/home/caq/anaconda3/bin/conda"
fi
if [[ -z "${CONDA_EXE}" ]]; then
...
PRETRAINED_FROM="${PRETRAINED_FROM:-/mnt/drive/3333_raw/checkpoints/depth_anything_v2_vitl.pth}"
...
INPUT_TYPE="${INPUT_TYPE:?INPUT_TYPE is required}"
DAV2_TRAIN_MODE="${DAV2_TRAIN_MODE:?DAV2_TRAIN_MODE is required}"
...
BRIDGE_FEATURE_KEYS_STR="${BRIDGE_FEATURE_KEYS_STR:-x_cat ffm_mid x4}"
read -r -a BRIDGE_FEATURE_KEYS <<< "${BRIDGE_FEATURE_KEYS_STR}"

RUN_DIR="${RUN_DIR:-${EXP_ROOT}/${RUN_NAME}}"

EXTRA_ARGS=()
if [[ -n "${RGB_INTERFACE_MODE:-}" ]]; then
  EXTRA_ARGS+=(--rgb-interface-mode "${RGB_INTERFACE_MODE}")
fi
if [[ -n "${RGB_RESIDUAL_SCALE:-}" ]]; then
  EXTRA_ARGS+=(--rgb-residual-scale "${RGB_RESIDUAL_SCALE}")
fi
if [[ -n "${BACKBONE_LAYER_DECAY:-}" ]]; then
  EXTRA_ARGS+=(--backbone-layer-decay "${BACKBONE_LAYER_DECAY}")
fi
if [[ -n "${LORA_BLOCK_MODE:-}" ]]; then
  EXTRA_ARGS+=(--lora-block-mode "${LORA_BLOCK_MODE}")
fi
if [[ -n "${LORA_RANK:-}" ]]; then
...
if [[ "${EVAL_NYU}" == "1" ]]; then
  EXTRA_ARGS+=(--eval-nyu --nyu-dir "${NYU_DIR}")
...
"${CONDA_EXE}" run --live-stream -n "${CONDA_ENV}" torchrun --nproc_per_node="${GPUS}" --master_port "${MASTER_PORT}" finetune_stf/train.py \
  --stage vkitti_lod \
  --input-type "${INPUT_TYPE}" \
  --encoder "${ENCODER}" \
  --dav2-train-mode "${DAV2_TRAIN_MODE}" \
  --loss-type ssi \
