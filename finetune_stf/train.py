...
def parse_args():
    ...
    parser.add_argument("--rod-student-white-percentile", default=STUDENT_WHITE_PERCENTILE, type=float)
    ...
    if uses_rod_dataset(args):
        ...
        if args.rod_raw_source != "raw24":
            parser.error("ROD v1 requires --rod-raw-source raw24")
        if args.rod_label_space != "inverse_relative":
            parser.error("ROD v1 requires --rod-label-space inverse_relative")
        if uses_rod_raw_student_rgb_dataset(args):
            if args.rod_rgb_pipeline != "student_dark_degreen_v1":
                parser.error("ROD student-RGB v1 requires --rod-rgb-pipeline student_dark_degreen_v1")
            if abs(args.rod_student_white_percentile - STUDENT_WHITE_PERCENTILE) > 1e-6:
                parser.error("ROD student-RGB v1 requires --rod-student-white-percentile 99.9")
            if abs(args.rod_student_gamma - STUDENT_GAMMA) > 1e-6:
        ...
        if not rod_root.is_dir():
            parser.error(f"--rod-root does not exist or is not a directory: {rod_root}")
        if not rod_manifest.is_file():
...
def log_resolved_summary(logger, args):
    cfg = resolved_config(args)
    logger.info(
        "[RESOLVED] input_domain=%s front_end=%s model_input_tensor=%s dataset_family=%s dataset_input_mode=%s",
        cfg.input_domain,
        cfg.front_end,
...
def build_datasets(args):
    ...
    if cfg.dataset_family in {"rod_raw_student_rgb", "rod_raw"}:
        ...
        if not args.eval_only:
            datasets["rod_train"] = rod_dataset_cls(
                split="00Train",
                mode="train",
...
def build_dataloaders(args, datasets):
    ...
    if args.eval_only:
        ...
        if "rod_train" in datasets:
            ...
            train_key = "stf_train"
            source_name = "stf"
        train_sampler = DistributedSampler(datasets[train_key], shuffle=True)
        ...
        state["samplers"][source_name] = train_sampler
        state["steps_per_epoch"] = len(train_loader)
        state["single_source"] = source_name
        state["source_names"] = (source_name,)
...
def log_setup(logger, args, datasets, train_state, model):
    ...
    elif not uses_rod_dataset(args):
        if "val" in datasets:
            logger.info(
    ...
    if "rod_train" in datasets:
        if "val" in datasets:
            logger.info(
                "[DATASET] rod_train=%d rod_night_val=%d",
                len(datasets["rod_train"]),
                len(datasets["val"]),
            )
        else:
    ...
    if cfg.front_end == "raw_to_base_rgb_ram3" and not uses_bridge(args) and not uses_decoder_feature_adapter(args):
        ...
        if cfg.dataset_family == "rod_raw":
            logger.info("[MODEL] raw4 -> [R,(Gr+Gb)/2,B] -> RamCore3 -> %s -> DAv2", args.raw_ram_rgb_tail)
        logger.info(
    ...
    *,
    ...
    for sample in valloader:
        ...
        if target_space == "inverse_relative":
            valid_mask = valid_mask & torch.isfinite(depth) & (depth > 0)
            ...
            aligned_inverse, _ = affine_align_to_inverse_target(pred_np, depth_np, valid_np)
            metrics = compute_inverse_relative_metrics(aligned_inverse, depth_np, valid_np)
        else:
...
def main():
    ...
    if args.eval_only:
        if valloader is not None:
            eval_only_tag = f"eval_only_{primary_eval_name}"
